...
## 18. Почему контент из темы попадает на сайт

В BWWH подключена локальная тема:

```yaml
theme: basic
```

У темы есть свои файлы:

```text
themes/basic/layouts/
themes/basic/content/
themes/basic/assets/
```

Важно: Hugo может использовать не только файлы из корня проекта, но и файлы из подключенной темы.
...
тоже становится страницей сайта, потому что это контент внутри темы.

Для учебного проекта это полезно:

- можно увидеть, как Hugo собирает страницы из разных источников;
- можно изучить `range site.RegularPages`;
- можно увидеть, как работают tags и taxonomy pages;
- можно разобрать page bundle на примере `post-3/index.md` и картинки рядом с ним.

Для production-сайта обычно делают иначе:

- demo content из темы удаляют;
- реальные страницы держат в корневой папке `content/`;
- тему используют в основном для `layouts`, `partials`, `assets` и стилей.

В BWWH demo posts оставлены специально и названы `Learning Example`, чтобы было понятно: это учебные примеры, а не настоящий контент сайта.
